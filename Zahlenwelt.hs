...
moeglich :: Zahlenwelt -> Handlung Zahlenwelt -> Bool
moeglich (Zahlenwelt verbleibend welt) h =
  let (Zahlenwelt v w) = handeln (Zahlenwelt verbleibend welt) h in
  v >= 0
...
maxime_zahlenfortschritt = Kant.Maxime $ \w (Handlung h) -> fortschritt w (h w)
...
beispiel_kategorischer_imperativ = Kant.kategorischer_imperativ (Zahlenwelt 9000 0) (Handlung (abbauen 5)) maxime_zahlenfortschritt case_law_ableiten leer

-- max i iterations
make_case_law :: Int -> Handlung Zahlenwelt -> Zahlenwelt -> CaseLaw Zahlenwelt -> CaseLaw Zahlenwelt
...
make_case_law i h w g =
  ...
  let w' = (if s == Erlaubnis then handeln w h else w) in
  make_case_law (i-1) h w' g'

beispiel = make_case_law 100 (Handlung (abbauen (-1))) (Zahlenwelt 42 5) zahlengesetz_beispiel
