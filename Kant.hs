...
type Zahlengesetz = Gesetz (Integer, Integer) (Sollensanordnung)

show_zahlengesetz :: Zahlengesetz -> String
show_zahlengesetz (Gesetz g) = S.foldl (\s p-> s ++ show_paragraph p ++ "; ") "" g
  where
    show_paragraph (Paragraph p, rechtsnorm) = p ++ ": " ++ show_zahlenrechtsnorm rechtsnorm
    show_zahlenrechtsnorm (Rechtsnorm (Tatbestand (a,b)) (Rechtsfolge f)) = "Wenn die welt " ++ show a ++ " ist und wir die welt nach " ++
                                                                            show b ++ " aendern wollen, dann " ++ show f

zahlengesetz_leer :: Zahlengesetz
zahlengesetz_leer = Gesetz $ S.empty

zahlengesetz_beispiel :: Zahlengesetz
zahlengesetz_beispiel = Gesetz $ S.singleton ((Paragraph "Paragraph 42"), (Rechtsnorm (Tatbestand (1,2)) (Rechtsfolge Verbot)))
